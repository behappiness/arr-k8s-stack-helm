...
*/}}
{{- define "emby.configClaimName" -}}
{{- if .Values.persistence.config.existingClaim }}
{{- .Values.persistence.config.existingClaim }}
{{- else }}
{{- printf "%s-config" (include "emby.fullname" .) }}
{{- end }}
{{- end }}
...
Value validation.
*/}}
{{- define "emby.validateValues" -}}
{{- if gt (int .Values.replicaCount) 1 }}
{{- fail "emby: replicaCount must be 1. Emby is a single-instance application backed by an RWO volume; running multiple replicas corrupts its database." }}
{{- end }}
{{- if and .Values.persistence.media.enabled (not .Values.persistence.media.existingClaim) (not .Values.persistence.media.size) }}
{{- fail "emby: persistence.media.enabled is true but neither persistence.media.existingClaim nor persistence.media.size is set." }}
{{- end }}
{{- end }}
