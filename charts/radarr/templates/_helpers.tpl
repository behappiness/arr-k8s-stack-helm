...
*/}}
{{- define "radarr.envPrefix" -}}RADARR{{- end }}
...
Value validation.
*/}}
{{- define "radarr.validateValues" -}}
{{- if gt (int .Values.replicaCount) 1 }}
{{- fail "radarr: replicaCount must be 1. Radarr is a single-instance application backed by an RWO volume; running multiple replicas corrupts its database." }}
{{- end }}
{{- if and .Values.persistence.media.enabled (not .Values.persistence.media.existingClaim) (not .Values.persistence.media.size) }}
{{- fail "radarr: persistence.media.enabled is true but neither persistence.media.existingClaim nor persistence.media.size is set. Set one of them." }}
{{- end }}
{{- end }}
