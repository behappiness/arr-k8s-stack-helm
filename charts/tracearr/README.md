...
![Version: 0.1.6](https://img.shields.io/badge/Version-0.1.6-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 2.2.3](https://img.shields.io/badge/AppVersion-2.2.3-informational?style=flat-square)
...
| config.cookieSecret | string | `""` | Secret signing key for cookies (`COOKIE_SECRET`). Generated when empty and preserved across upgrades. |
| config.databaseUrl | string | `""` | PostgreSQL connection string (`DATABASE_URL`), e.g. `postgres://tracearr:password@timescale.db.svc:5432/tracearr`. Prefer `existingSecret` - a value here ends up in the release manifest. |
| config.existingSecret | string | `""` | Name of an existing Secret holding any of the four values above. Keys: `database-url`, `redis-url`, `jwt-secret`, `cookie-secret`. |
| config.jwtSecret | string | `""` | Secret signing key for sessions (`JWT_SECRET`). Generated when empty and preserved across upgrades. |
| config.logLevel | string | `"info"` | Log level (`LOG_LEVEL`): `error`, `warn`, `info` or `debug` |
| config.redisUrl | string | `""` | Redis connection string (`REDIS_URL`), e.g. `redis://redis.db.svc:6379` |
...
| image.tag | string | `"2.2.3"` | Image tag. Pin this in production. |
| imagePullSecrets | list | `[]` | Image pull secrets |
| ingress.annotations | object | `{}` | Ingress annotations |
| ingress.className | string | `""` | IngressClass name |
| ingress.enabled | bool | `false` | Create an Ingress |
| ingress.hosts | list | `[{"host":"tracearr.local","paths":[{"path":"/","pathType":"Prefix"}]}]` | Ingress hosts |
...
| persistence.config.enabled | bool | `true` | Persist the application's data directory (`/app/data`) |
...
| resources | object | `{"limits":{"memory":"1Gi"},"requests":{"cpu":"50m","memory":"256Mi"}}` | Resource requests and limits |
| runtimeClassName | string | `""` | Runtime class name |
| securityContext | object | `{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"readOnlyRootFilesystem":false,"runAsGroup":1000,"runAsNonRoot":true,"runAsUser":1000}` | Container-level security context. This image already runs as a non-root user, so the chart keeps it that way and drops every capability. |
...
| service.port | int | `3000` | Service port. The container always listens on 3000. |
