...
*/}}
{{- define "jellyfin.configClaimName" -}}
{{- if .Values.persistence.config.existingClaim }}
{{- .Values.persistence.config.existingClaim }}
{{- else }}
{{- printf "%s-config" (include "jellyfin.fullname" .) }}
{{- end }}
{{- end }}
...
{{- fail "jellyfin: replicaCount must be 1. Jellyfin is a single-instance application backed by an RWO volume; running multiple replicas corrupts its database." }}
{{- end }}
{{- if and .Values.persistence.media.enabled (not .Values.persistence.media.existingClaim) (not .Values.persistence.media.size) }}
{{- fail "jellyfin: persistence.media.enabled is true but neither persistence.media.existingClaim nor persistence.media.size is set. Set one of them." }}
{{- end }}
{{- end }}
