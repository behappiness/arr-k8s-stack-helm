...
{{- define "jellyfin.hwaccel.volumes" -}}
...
{{- define "jellyfin.hwaccel.volumeMounts" -}}
{{- if eq .Values.hardwareAcceleration.mode "hostDevice" }}
- name: dri
...
{{- define "jellyfin.hwaccel.resources" -}}
...
{{- define "jellyfin.hwaccel.podResourceClaims" -}}
{{- if eq .Values.hardwareAcceleration.mode "dra" }}
- name: gpu
  resourceClaimTemplateName: {{ printf "%s-gpu" (include "jellyfin.fullname" .) }}
{{- end }}
{{- end }}

{{/* Container-level claim reference, for DRA. */}}
{{- define "jellyfin.hwaccel.containerResourceClaims" -}}
{{- if eq .Values.hardwareAcceleration.mode "dra" }}
claims:
  - name: gpu
{{- end }}
...
{{/* Validation. */}}
{{- define "jellyfin.hwaccel.validate" -}}
{{- $mode := .Values.hardwareAcceleration.mode -}}
{{- if not (has $mode (list "none" "hostDevice" "devicePlugin" "dra")) }}
{{- fail (printf "jellyfin: hardwareAcceleration.mode must be one of none, hostDevice, devicePlugin or dra (got %q)" $mode) }}
{{- end }}
{{- if and (eq $mode "devicePlugin") (not .Values.hardwareAcceleration.devicePlugin.resource) }}
{{- fail "jellyfin: hardwareAcceleration.mode is \"devicePlugin\" but hardwareAcceleration.devicePlugin.resource is empty. Set the resource name the plugin advertises, e.g. gpu.intel.com/i915." }}
{{- end }}
{{- if and (eq $mode "dra") (not .Values.hardwareAcceleration.dra.deviceClassName) }}
{{- fail "jellyfin: hardwareAcceleration.mode is \"dra\" but hardwareAcceleration.dra.deviceClassName is empty, e.g. gpu.nvidia.com." }}
{{- end }}
{{- if and (eq $mode "dra") (semverCompare "<1.34-0" (.Capabilities.KubeVersion.Version | trimPrefix "v")) }}
{{- fail (printf "jellyfin: hardwareAcceleration.mode is \"dra\", which needs resource.k8s.io/v1 (Kubernetes 1.34 or newer). This cluster reports %s." .Capabilities.KubeVersion.Version) }}
{{- end }}
{{- end }}
...
*/}}
{{- define "jellyfin.hwaccel.securityContext" -}}
